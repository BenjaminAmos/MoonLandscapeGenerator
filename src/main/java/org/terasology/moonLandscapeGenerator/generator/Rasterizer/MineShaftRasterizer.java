...
public class MineShaftRasterizer implements WorldRasterizer {
    ...
    public void generateChunk(CoreChunk chunk, Region chunkRegion) {
        MineShaftFacet houseFacet = chunkRegion.getFacet(MineShaftFacet.class);

        for (Entry<BaseVector3i, MineShaft> entry : houseFacet.getWorldEntries().entrySet()) {
            ...
            for (Vector3i newBlockPosition : walls) {
                if (chunkRegion.getRegion().encompasses(newBlockPosition) && inside.encompasses(newBlockPosition) && newBlockPosition.x == centerHousePosition.x && newBlockPosition.z == centerHousePosition.z) {
